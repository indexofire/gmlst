...
ONT 读长与其他 FASTQ 输入一样走 KMA 路径。基于与配套混合组装（霍乱弧菌与副溶血弧菌，R10.4/Q20+ 数据）的实测验证：

- **经典 7 基因座 MLST 大部分可直接从 reads 调用**：7 个基因座中 5-6 个精确匹配，其余以最接近（`~n`）调用报告，而非错误的精确调用。
- **单基因座翻转可能发生**：甲基化位点碱基识别错误（ONT 已知伪影，原生 DNA 文库尤其明显）可能使某个等位基因编号在重复运行或与组装真值比较时发生变化。reads 与组装结果之间的单基因座差异，在确认是生物学差异之前应保持怀疑。
- **我们测试了 KMA 的 `-bcNano` 模式但未启用**：在 Q20+ 验证对上输出与默认模式完全一致——它针对缺失型误差，而甲基化误差是替换型。

建议：

- 需要权威 ST 结论时，先组装（如 Flye + Medaka），再对组装 FASTA 走常规分型路径。
- reads 直接分型适合快速筛查；对 novel 或低置信调用用 `--format json` 的分数复核或做定向二次分析。
- 不建议对 ONT reads 不经组装直接做 cgMLST——误差会在数千个基因座上累积。
